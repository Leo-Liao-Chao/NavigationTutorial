...
public:
  ...
  ~AstarPlanner(){}
  AstarPlanner(string name, costmap_2d::Costmap2DROS* costmap_ros)
  {
    isInitilized = false;
  ...
  void initialize(string name, costmap_2d::Costmap2DROS* costmap_ros)
  {
    if(isInitilized)
    {
      ROS_WARN("AstarPlanner global planner: Already initilized");
      return;
    }
    costmapROS = costmap_ros;
    costmap = costmap_ros->getCostmap();
    ros::NodeHandle nh("~/AstarPlanner");
    guiPathPub = nh.advertise<nav_msgs::Path>("plan", 1);
    debugPub = nh.advertise<std_msgs::String>("debug_info", 1); // 添加调试信息的publisher
    isInitilized = true;
    ROS_INFO("AstarPlanner global planner: Initialized successfully");
  }
  ...
  const double getMoveCost(int firstIndex, int secondIndex) const
  {
    ...
    {
      ROS_ERROR("AstarPlanner global planner: Error in getMoveCost - difference not valid");
      return 1.0;
    }
    if(difference == 1)
    return 1.0;
  ...
  bool makePlan(const geometry_msgs::PoseStamped& start,
    ...
    std::vector<geometry_msgs::PoseStamped>& plan)
    {
      if(isInitilized == false){
        ROS_ERROR("AstarPlanner global planner: Initilize must be called first");
        return false;
      }
      ...
      ROS_DEBUG("AstarPlanner global planner: Start cords (%.2f,%.2f) and goal cords (%.2f,%.2f)",
      ...
      std_msgs::String msg;
    msg.data = "Path planning successful.";
  ...
  PLUGINLIB_EXPORT_CLASS(AstarPlanner, nav_core::BaseGlobalPlanner);
