...
Obstacle::Obstacle(const Parameters &p)
{
    this->p = p;
}

Obstacle::Obstacle(const Parameters &p, const Eigen::MatrixXd &dimension, const Eigen::MatrixXd &relative_pos_array)
{
    this->p = p;
...
mx:[x,x;x,x;] 2*2
*/
struct_x_vx_mx_vmx Obstacle::barrier_function(const double &q1, const double &q2, const double &c, const Eigen::Vector4d &c_dot)
{
...
mx:4*4
*/
struct_x_vx_mx_vmx Obstacle::get_obstalce_cost(const int &i, const Eigen::VectorXd &ego_state)
{
    Eigen::VectorXd obstalce_pos = this->relative_pos_array.col(i);
    double a = this->dimension(0, i) + std::abs(obstalce_pos(2) * std::cos(obstalce_pos(3))) * this->p.t_safe + this->p.s_safe_a + this->p.ego_rad;
    double b = this->dimension(1, i) + std::abs(obstalce_pos(2) * std::sin(obstalce_pos(3))) * this->p.t_safe + this->p.s_safe_b + this->p.ego_rad;
